            ...
            if (len > 0) {
                ...
                for (var i = 0, startLen = startDate.length; i < startLen; i++) {
                    ...
                    if (date == targetdate) {
                        ...
                        show = !!flag;
                        if (show) pricetext = "<span class='price'><em>可售</em></span>";
                        break;
                    } 
        ...
        today: function (startYear, month, opts) {
            ...
            return $('<div>', {
                ...
                $.ajax({
                    url: opts.url,
                    async: false,
                    data: $.extend(opts.param, { "month": calenderMonth }),
                    type: "get",
                    success: function (data) {
                        if (data.status === 200) {
                            console.log(data.data.showResponseDto.sessionMonthFlagList)
                            opts.settingdata = data.data.showResponseDto.sessionMonthFlagList;
                        }
